...
export async function createSession(token: string, userId: number) {
	...
	await db.insert(table.userSessions).values(session);
	return session;
}
...
export async function validateSessionToken(token: string) {
	...
	const [result] = await db
		.select({
			user: {
				...
				type: table.users.type,
				language: table.users.language,
				theme: table.users.theme,
				defaultLocation: table.users.defaultLocation
			},
	...
	if (sessionExpired) {
		await db.delete(table.userSessions).where(eq(table.userSessions.id, session.id));
		return { session: null, user: null };
	}
...
export async function invalidateSession(sessionId: string) {
	await db.delete(table.userSessions).where(eq(table.userSessions.id, sessionId));
}
